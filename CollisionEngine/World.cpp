...
CPolygonPtr		CWorld::AddRandomPoly(const SRandomPolyParams& params)
{
	...
	poly->Build();
	poly->transform.SetAngle(Random(-180.0f, 180.0f));
	poly->transform.setPosition(Vec2(Random(params.minBounds.x, params.maxBounds.x), Random(params.minBounds.y, params.maxBounds.y)));
	Mat2 rot;
