	...
	virtual void Update(float frameTime) override
	{
		if (gVars->pRenderWindow->GetMouseButton(0) || gVars->pRenderWindow->GetMouseButton(2))
		{
			if (!m_selectedPoly)
			{
				...
				if (m_selectedPoly)
				{
					m_clickAngle = m_selectedPoly->transform.GetAngle();
				}
			}
			else
			{
				...
				if (m_translate)
				{
					m_selectedPoly->transform.updatePosition(mousePoint - m_prevMousePos);
					m_selectedPoly->speed = Vec2();
				}
				else
				{
					Vec2 from = m_clickMousePos - m_selectedPoly->transform.GetPosition();
					Vec2 to = mousePoint - m_selectedPoly->transform.GetPosition();

					m_selectedPoly->transform.SetAngle(m_clickAngle + from.Angle(to));
					m_selectedPoly->speed = Vec2();
				}
